"""Metricas - Volume, trends, engagement analytics."""
...
# Sidebar (clinical filters only, no date/curated/limit)
...
col3.metric("Autores Unicos", f"{df['authors'].sum():,}")
col4.metric("Dias Coletados", f"{len(df)}")
...
fig_tweets.update_layout(xaxis_tickangle=-45, height=350)
...
fig_eng.update_layout(
    ...
    xaxis_title="Data", yaxis_title="Count", height=350,
)
...
fig_auth.update_layout(height=300)
...
with st.expander("Dados brutos"):
    ...
    if st.button("Exportar CSV"):
        try:
            from src.config import get_exports_dir
            export_path = export_tweets_csv(get_db_path())
